...
export async function syncPricingManifest(
  ...
  if (!options?.force && Date.now() - lastChecked < ONE_DAY_MS && cachedManifest) {
    return { manifest: cachedManifest, source: "cache" };
  }
  ...
  try {
    ...
    if (!response) {
      return { manifest: cachedManifest ?? BUNDLED_PRICING, source: cachedManifest ? "cache" : "bundled" };
    }
...
export function getCachedPricingManifest(store: PricingStore): PricingManifest {
  const cached = store.get(CACHE_KEY);
  return isPricingManifest(cached) ? cached : BUNDLED_PRICING;
}
